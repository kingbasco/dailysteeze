@if ($products instanceof \Illuminate\Contracts\Pagination\LengthAwarePaginator && $products->hasMorePages())
    <div class="wd-full justify-content-center mt-4" data-bb-toggle="load-more-wrapper">
        <button
            type="button"
            id="loadMoreBtn"
            class="btn-loadmore tf-btn animate-btn tf-loading loadmore"
            data-action="load-more"
            data-url="{{ $products->withQueryString()->nextPageUrl() }}"
            data-current-page="{{ $products->currentPage() }}"
            data-last-page="{{ $products->lastPage() }}"
        >
            <span class="text">{{ __('Load More') }}</span>
