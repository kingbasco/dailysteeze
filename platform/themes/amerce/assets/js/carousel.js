function initTfSwiper(scope) {
    var $scope = scope ? $(scope) : $(document);
    var $swipers = $scope.is(".tf-swiper") ? $scope : $scope.find(".tf-swiper");

    $swipers.each(function (index, element) {
        var $this = $(element);

        if ($this.data("tf-swiper-initialized")) {
            return;
        }
        ...
        // Disable loop when only one slide exists — Swiper otherwise clones the slide
        // into prev/next positions, which inflates the LCP image count for hero sliders.
        var loop = ($this.data("loop") ?? false) && $this.find(".swiper-slide").length > 1;
        var loopMd = ($this.data("loop-md") ?? false) && $this.find(".swiper-slide").length > 1;
...
window.amerceInitSwipers = initTfSwiper;

$(window).on("load", function () {
    initTfSwiper(document);
});

/* Observe DOM for AJAX-injected cross-sale blocks (plugin uses jQuery replaceWith).
   Replaces an inline <script> bootstrap that previously lived in
   views/ecommerce/includes/cross-sale-products.blade.php. */
if (typeof MutationObserver !== "undefined") {
    var crossSaleObserver = new MutationObserver(function (mutations) {
        for (var i = 0; i < mutations.length; i++) {
            ...
            for (var j = 0; j < added.length; j++) {
                ...
                var swiperEl = null;
                if (node.matches && node.matches(".ec-cross-sale-section")) {
                    swiperEl = node.querySelector(".tf-swiper:not([data-tf-swiper-initialized])");
                } else if (node.querySelector) {
                    swiperEl = node.querySelector(".ec-cross-sale-section .tf-swiper:not([data-tf-swiper-initialized])");
                }
                if (swiperEl && typeof window.amerceInitSwipers === "function") {
                    window.amerceInitSwipers(swiperEl);
                }
            }
        }
    });
    crossSaleObserver.observe(document.body, { childList: true, subtree: true });
}

if ($(".tf-sw-thumbs").length > 0 && $(".sw-thumb").length > 0 && $(".sw-main-thumb").length > 0) {
