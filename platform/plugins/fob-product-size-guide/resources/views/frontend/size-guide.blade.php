@if($sizeGuide)
    ...
    <div class="product-size-guide-wrapper mt-4" id="product-size-guide">
        ...
        @else
            {{-- Popup/Modal display: theme renders its own trigger above the
                 size attribute swatches (see views/ecommerce/attributes/swatches-renderer.blade.php),
                 so we suppress the plugin's auto-injected button to avoid a
                 duplicate "Size Guide" button between Size and Quantity. --}}
        @endif
    </div>
