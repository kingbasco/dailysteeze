...
/**
 * Overrides the base ReviewSeeder so each review's `images` field uses the
 * reviewing customer's avatar instead of random product images. Keeps the
 * gallery on the storefront review block visually consistent with the avatar
 * shown next to the reviewer name.
 */
class ReviewSeeder extends BaseReviewSeeder
{
    public function run(): void
    {
        ...
        $reviews = $this->getReviewComments();
        ...
        for ($attempt = 0; $attempt < $maxAttempts && $created < $target; $attempt++) {
            ...
            $avatar = $customers[$customerId]->avatar;
     ...
     */
    protected function getReviewComments(): array
    {
        return [
